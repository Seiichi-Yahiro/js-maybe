...
describe('Maybe', () => {
    ...
    it('should create a none', () => {
        [Maybe.of(null), Maybe.of(undefined), Maybe.none()].forEach(maybe => expect(maybe.isNone()).toBeTruthy());
    });

    it('should get the value if some and throw error if none', () => {
        expect(Maybe.some(5).get()).toEqual(5);
        expect(() => Maybe.none().get()).toThrowError('Provided value must not be empty');
    });
